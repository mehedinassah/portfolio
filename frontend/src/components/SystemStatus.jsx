  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-accent-slate/5 border border-accent-slate/20 backdrop-blur-sm">
      <div className="flex items-center gap-2">
        {isWorkingHours ? <CheckCircle2 size={14} className="text-emerald-500 animate-pulse-soft" /> : <Circle size={14} className="text-accent-slate" />}
        <span className="text-xs font-mono text-white/60">{isWorkingHours ? "Available" : "Offline"}</span>
      </div>
      <div className="w-px h-3 bg-accent-slate/20" />
      <span className="text-xs font-mono text-accent-slate-light">{hours}:{minutes} {isWorkingHours ? "AM" : "PM"}</span>
    </div>
  );
}
